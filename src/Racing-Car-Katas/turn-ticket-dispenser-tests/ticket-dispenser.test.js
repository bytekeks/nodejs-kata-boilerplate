// TicketDispenser exercise:
// write the unit tests for the TicketDispenser.
// The TicketDispenser class is designed to be used to manage
// a queuing system in a shop. There may be more than one
// ticket dispenser but the same ticket should not be issued
// to two different customers.

describe('Turn Ticket Dispenser', function () {

	describe('TurnTicketDispenser', function () {

		it('Do something', function () {

			var target = new TicketDispenser();

		});

	});

});
